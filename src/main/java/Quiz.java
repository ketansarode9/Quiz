...
class  QuizProg extends JFrame implements ActionListener{
            JPanel panel;
            JPanel panelresult;
       ...
       public class Report extends JFrame{
                        Report(String id){
                                    ...
                                    int ans = calCorrectAnswer() ;
                                    String scoreq = Integer.toString(ans) ;
                                     try{  
                                    Class.forName("com.mysql.jdbc.Driver");  
                                    Connection con=DriverManager.getConnection(  
                                    "jdbc:mysql://localhost:3306/studentdb","root","1234");  
                                     PreparedStatement stmt=con.prepareStatement("update student set score=? where userid=?");  
                                    stmt.setString(1,scoreq);//1 specifies the first parameter in the query i.e. name  
                                    stmt.setString(2,id);  
                                    int i=stmt.executeUpdate();  
                                    System.out.println(i+" score records updated");  
                                    con.close();  
